...
class detector(nn.Module):
    ...
    def __init__(self, train, object_classes, use_SUPPLY, mode='predcls', device=None):
        ...
        checkpoint = torch.load('fasterRCNN/models/faster_rcnn_ag.pth', map_location=self.device)
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, gt_annotation, im_all):
        ...
        if self.mode == 'sgdet':
            ...
            FINAL_BBOXES = torch.tensor([]).cuda(self.device)
            FINAL_LABELS = torch.tensor([], dtype=torch.int64).cuda(self.device)
            FINAL_SCORES = torch.tensor([]).cuda(self.device)
            FINAL_FEATURES = torch.tensor([]).cuda(self.device)
            FINAL_BASE_FEATURES = torch.tensor([]).cuda(self.device)

            while counter < im_data.shape[0]:
                #compute 10 images in batch and  collect all frames dataset in the video
                ...
                box_deltas = box_deltas.view(-1, 4) * torch.FloatTensor([0.1, 0.1, 0.2, 0.2]).cuda(self.device) \
                             + torch.FloatTensor([0.0, 0.0, 0.0, 0.0]).cuda(self.device)  # the first is normalize std, the second is mean
                ...
                for i in range(rois.shape[0]):
                    ...
                    for j in range(1, len(self.object_classes)):
                        ...
                        if inds.numel() > 0:
                            ...
                            keep = torchvision.ops.nms(cls_boxes[order, :], cls_scores[order], 0.4) # NMS threshold
                            ...
                            if j == 1:
                                ...
                                final_labels = torch.tensor([j]).cuda(self.device)
                                final_features = roi_features[i, inds[order[keep][0]]].unsqueeze(0)
                            else:
                                final_bbox = cls_dets[:, 0:4]
                                final_score = cls_dets[:, 4]
                                final_labels = torch.tensor([j]).repeat(keep.shape[0]).cuda(self.device)
                                final_features = roi_features[i, inds[order[keep]]]

                            final_bbox = torch.cat((torch.tensor([[counter_image]], dtype=torch.float).repeat(final_bbox.shape[0], 1).cuda(self.device),
                                                    final_bbox), 1)
            ...
            if self.is_train:
                ...
                if self.use_SUPPLY:
                    ...
                    FINAL_BBOXES_X = torch.tensor([]).cuda(self.device)
                    FINAL_LABELS_X = torch.tensor([], dtype=torch.int64).cuda(self.device)
                    FINAL_SCORES_X = torch.tensor([]).cuda(self.device)
                    FINAL_FEATURES_X = torch.tensor([]).cuda(self.device)
                    ...
                    for i, j in enumerate(SUPPLY_RELATIONS):
                        if len(j) > 0:
                            unfound_gt_bboxes = torch.zeros([len(j), 5]).cuda(self.device)
                            unfound_gt_classes = torch.zeros([len(j)], dtype=torch.int64).cuda(self.device)
                            one_scores = torch.ones([len(j)], dtype=torch.float32).cuda(self.device)  # probability
                            for m, n in enumerate(j):
                                # if person box is missing or objects
                                if 'bbox' in n.keys():
                                    unfound_gt_bboxes[m, 1:] = torch.tensor(n['bbox']).to(self.device) * im_info[
                                        i, 2]  # don't forget scaling!
                                    unfound_gt_classes[m] = n['class']
                                else:
                                    # here happens always that IOU <0.5 but not unfounded
                                    unfound_gt_bboxes[m, 1:] = torch.tensor(n['person_bbox']).to(self.device) * im_info[
                                        i, 2]  # don't forget scaling!
                            ...
                            pooled_feat = self.ROIAlign(FINAL_BASE_FEATURES[i].unsqueeze(0),
                                                                         unfound_gt_bboxes.cuda(self.device))
                ...
                HUMAN_IDX = torch.zeros([im_data.shape[0], 1], dtype=torch.int64).cuda(self.device)
                ...
                pair = torch.tensor(pair).cuda(self.device)
                im_idx = torch.tensor(im_idx, dtype=torch.float).cuda(self.device)
                ...
                union_feat = self.ROIAlign(FINAL_BASE_FEATURES, union_boxes)
            ...
            for i in gt_annotation:
                bbox_num += len(i)
            FINAL_BBOXES = torch.zeros([bbox_num,5], dtype=torch.float32).cuda(self.device)
            FINAL_LABELS = torch.zeros([bbox_num], dtype=torch.int64).cuda(self.device)
            FINAL_SCORES = torch.ones([bbox_num], dtype=torch.float32).cuda(self.device)
            HUMAN_IDX = torch.zeros([len(gt_annotation),1], dtype=torch.int64).cuda(self.device)
            ...
            # add
            FINAL_FEATURES = self.ROIAlign(FINAL_BASE_FEATURES, FINAL_BBOXES)
