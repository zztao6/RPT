...
class AG(Dataset):
    ...
    def __init__(self, mode, datasize, data_path=None, filter_nonperson_box_frame=True, filter_small_box=False):
        ...
        if filter_small_box:
            ...
            with open(root_path+'annotations/object_bbox_and_relationship.pkl', 'rb') as f:
                object_bbox = pickle.load(f)
            f.close()
        with open('dataloader/video_list_with_face_0.9.pkl', 'rb') as f:
            video_list_with_face = pickle.load(f)
        f.close()
        print('--------------------finish!-------------------------')
        ...
        for i in person_bbox.keys():
            # collect frame with face confidence>0.9
            video_name, frame_num = i.split('/')
            if video_name not in video_list_with_face.keys():
                continue
            if frame_num not in video_list_with_face[video_name]:
                continue
        ...
        # print the max num of frame in video
        video_len = []
        for video in self.video_list:
            video_len.append(len(video))
        print("max num of frame in video： ", max(video_len))
    ...
    def __getitem__(self, index):
        ...
        return img_tensor, im_info, gt_boxes, num_boxes, index, origin_im, origin_name
