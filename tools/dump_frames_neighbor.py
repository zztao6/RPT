...
def dump_frames(args, frame_list_file='frame_list.txt', st_id=1):
    ...
    print("Create video to frames mapping")
    video2frames = {}
    ...
    print("For each video, dump frames")
        ...
        if not os.path.exists(curr_frame_dir):
            ...
            if frames_store_type == 'jpg' and args.high_quality:
                if ignore_editlist:
                    ...
                    os.system('ffmpeg -loglevel panic -i %s/%s -start_number %d -qscale:v 2 %s/%%06d.%s' % (
                    video_dir, v, st_id, curr_frame_dir, frames_store_type))

            else:
                if ignore_editlist:
                    os.system('ffmpeg -ignore_editlist 1 -loglevel panic -i %s/%s -start_number %d %s/%%06d.%s' % (
                    video_dir, v, st_id, curr_frame_dir, frames_store_type))
                else:
                    os.system('ffmpeg -loglevel panic -i %s/%s -start_number %d %s/%%06d.%s' % (
                    video_dir, v, st_id, curr_frame_dir, frames_store_type))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--frame_dir", default="ssd_data_disk/dataset/ag/sampled_frames",
